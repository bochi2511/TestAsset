        ...
        private void DeliverImage()
        {
            if (mTexture.didUpdateThisFrame)
            {
				...
				if (colors.Length == mImageBuffer.Length / 4)
                {
					...
					bool softwareFlip = true;

                    int rotation = mTexture.videoRotationAngle;

                    if (softwareFlip)
                    {
                        //no fast implementation of this yet.
                        //this is needed to convert unity format to WebRTC
                        //Unity has the top pixels of the image written on the end
                        //of the buffer.
                    ...
                    UpdateFrame(VideoType.kARGB, mImageBuffer, (uint)mImageBuffer.Length, mWidth, mHeight, rotation);
